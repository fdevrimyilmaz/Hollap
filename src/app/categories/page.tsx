...
const iconMap: Record<string, JSX.Element> = {
  code: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg>,
  ...
  music: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3" /></svg>,
  ...
  dumbbell: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>,
  utensils: <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
        ...
        <section className="relative pt-28 lg:pt-32 pb-12 overflow-hidden">
          ...
          <div className="container mx-auto px-4 relative z-10">
            ...
            <p className="text-pretty text-lg text-muted-foreground max-w-3xl leading-relaxed">
              Kategoriler, aktif ürün listesine göre canlı olarak güncellenir.
              İlgi alanına en uygun olanı seç ve doğrudan içeriklere ulaş.
            </p>
            {categories.length > 0 && (
              <div className="mt-6 flex flex-wrap gap-6 text-sm text-muted-foreground">
                <span><span className="text-white font-semibold tabular-nums">{categories.length}</span> kategori</span>
                <span><span className="text-white font-semibold tabular-nums">{totalContent}</span> içerik</span>
              </div>
            )}
          </div>
        </section>
          ...
          <div className="container mx-auto px-4">
            {categories.length > 0 ? (
                ...
                {categories.map((category) => (
                  <Link key={category.id} href={`/category/${category.id}`} className="group block">
                    <article className="glass-card rounded-2xl p-6 card-hover h-full">
                      <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${category.color} flex items-center justify-center text-white shadow-lg mb-4 group-hover:scale-110 transition-transform`}>
                        {iconMap[category.icon] ?? iconMap.briefcase}
                      </div>
                      <h2 className="text-base font-semibold text-white group-hover:text-orange-400 transition-colors">
                        {category.name}
                      </h2>
                      <p className="text-xs text-muted-foreground mt-1 tabular-nums">{category.count} içerik</p>
                    </article>
                  </Link>
                ))}
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-12 text-center max-w-xl mx-auto">
                ...
                <h2 className="text-lg font-semibold text-white mb-2">Kategori bulunamadı</h2>
                <p className="text-sm text-muted-foreground mb-5">
                  Aktif ürün eklendikçe kategoriler bu sayfada görünmeye başlayacak.
                </p>
                <Link href="/become-creator">
                  <Button className="gradient-bg text-white border-0 shadow-lg shadow-orange-500/25">
                    İlk içeriği sen ekle
                  </Button>
                </Link>
              </div>
            )}
